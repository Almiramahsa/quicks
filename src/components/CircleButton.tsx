...
const CircleButton: FC<CircleButtonProps> = ({ id, label, onClick, icon, buttonColor }) => {
    ...
    return (
      <div className="fixed-bottom">
        <button
          id={id}
          className={`flex items-center justify-center bg-${buttonColor} w-20 h-20 rounded-full focus:outline-none border-none fixed bottom-4 mb-8 mr-4 right-4 ${isOpen ? 'hidden' : ''}`}
          onClick={() => toggleMenu()}
        >
          <img
            src={icon}
